...
async function getMedia() {
  ...
  const res = await fetch(`${baseUrl}/api/media?limit=20&sort=-createdAt`, {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to load media: ${res.status} ${text}`)
  }
    ...
    <div>
      <div className="flex items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-600">Recent media uploads</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="grid grid-cols-12 gap-4 px-6 py-3 text-sm font-semibold text-gray-600 bg-gray-50 border-b border-gray-200">
          ...
          <div className="col-span-6">Alt</div>
        </div>
        <div className="divide-y divide-gray-100">
          {docs.length === 0 ? (
            <div className="px-6 py-10 text-gray-600">No media found.</div>
          ) : (
            docs.map((doc: any) => (
              <div key={doc.id} className="grid grid-cols-12 gap-4 px-6 py-4">
                <div className="col-span-6 font-medium text-gray-900">
                  {doc.filename || doc.file?.filename || doc.id}
                </div>
                <div className="col-span-6 text-gray-700">{doc.alt || ''}</div>
              </div>
            ))
          )}
        </div>
